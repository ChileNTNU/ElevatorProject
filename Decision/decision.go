...
//---------------MASTER
    ...
    var BackParticipant *list.Element
				...
				if(dummyElement.Direction == server.NONE) {
					for e := ParticipantsList.Front(); e != nil; e = e.Next(){
		                if(e.Value.(redundancy.Participant).IPsender == "Local"){
							if(fitInQueue(e.Value.(redundancy.Participant).GotoQueue,e.Value.(redundancy.Participant).ActualPos,dummyElement) == false){
                    ...
                    // determine the elevatores which go in the same direction 
                    DirectionQueueUp = 0
                    for e := ParticipantsList.Front(); e != nil; e = e.Next(){
                        FrontParticipant = e.Value.(redundancy.Participant).GotoQueue.Front()
                        BackParticipant = e.Value.(redundancy.Participant).GotoQueue.Back()
                        DirectionQueueUp = BackParticipant.Value.(server.ElementQueue).Floor - FrontParticipant.Value.(server.ElementQueue).Floor
                        if(DirectionQueueUp > 1) { 
                            DirectionQueueUp = server.UP
                        } else {
                            DirectionQueueUp = server.DOWN
                        }

	                    if(DirectionQueueUp == dummyElement.Direction){
                            SameDirectionParticipants.PushBack(e)
					    }						
	                }
                    ...
                    if(SameDirectionParticipants != nil) {
					    for e := SameDirectionParticipants.Front(); e != nil; e = e.Next(){
						    if(fitInQueue(e.Value.(redundancy.Participant).GotoQueue,e.Value.(redundancy.Participant).ActualPos,dummyElement) == true){
                                TargetParticipant = e.Value.(redundancy.Participant).IPsender
                                goto DecisionDone                                
                            }			
		                }
                    }
                    // new request does not fit in any queues that have the same direction -> take all
                    // Get Queue with closest end position
                    for e := ParticipantsList.Front(); e != nil; e = e.Next(){
                        BackParticipant = e.Value.(redundancy.Participant).GotoQueue.Back()
                        FloorDifference = BackParticipant.Value.(server.ElementQueue).Floor - dummyElement.Floor
...
func fitInQueue(List *list.List,ActualPos int,NewElement server.ElementQueue) bool {
	...
	for e := List.Front(); e != nil; e = e.Next(){
        if e.Value.(server.ElementQueue).Floor == NewElement.Floor {
            fmt.Println("Fit: already in list")
            return true
        }
    }
