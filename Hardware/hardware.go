...
func HardwareManager(ChanToServer chan<- server.ServerMsg, ChanFromRedundancy <-chan *list.List){
    ...
    go switchLights(ChanFromRedundancy, ChanToServer)
    ...
    for{
        select{
            case <- timeout:
                ...
                if(actual_sensor == -1){
                   ...
                   //FOR with a constant read of the sensor, and sleep for small interval sleep a little
                    for(actual_sensor == -1 && i < 500){
                        ...
                        i++;
                        if (DEBUG){fmt.Println("HW_ Looking for floor: ",actual_sensor, direction_speed, ActualPos)}
                    }
    	        	...
    	        	//If you are in a floor, then write the position to the server
                    dummyElement.Floor = actual_sensor
                    dummyElement.Direction = server.NONE
                ...
                //The first element is only going to be removed when we reache the floor
...
func switchLights(ChanFromRedundancy <-chan *list.List, ChanToServer chan<- server.ServerMsg){
    ...
    //Switch local light depending on our GotoQueue
    for{
        select{
            case GlobalLights =<- ChanFromRedundancy:
                ...
                //First switch off all the lamps
                ...
                for i := 0; i < server.FLOORS; i++{
                    ...
                    C.elev_set_button_lamp(C.BUTTON_COMMAND, C.int(i), 0)
                }
                ...
                //Read the go to queue and switch on local lights
                MsgToServer.Cmd = server.CMD_READ_ALL
                MsgToServer.QueueID = server.ID_GOTOQUEUE
                MsgToServer.ChanVal = nil
                ...
                ChanToServer <- MsgToServer
	            GotoQueue =<- ChanToServer_Hardware_Queue

	            for e := GotoQueue.Front(); e != nil; e = e.Next(){
                    if (e.Value.(server.ElementQueue).Direction == server.NONE){
                        C.elev_set_button_lamp(C.BUTTON_COMMAND, C.int(e.Value.(server.ElementQueue).Floor), 1)
                    }
                }
